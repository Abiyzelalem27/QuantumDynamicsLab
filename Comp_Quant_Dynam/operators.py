...
def n_proj_operator_sparse(local_dims, idx, n):
    """ Returns the projector operator for the `n`-th state of the `idx`-th party in a system with local dimensions given by `local_dims` as a sparse matrix.
    ...
    proj_op = diagonal_op_sparse(proj_arr.flatten()) # create a sparse diagonal matrix with the non-zero element on the diagonal to the n-th state
